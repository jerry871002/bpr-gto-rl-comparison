...
# agentR = StationaryOpponent(env_width=env.width, env_height=env.height, env_goal_size=env.goal_size, type_attack=4, type_defense=2)
...
for i in range(EPISODES):
    ...
    stateL, stateR = state_each(normalize(state)) # for M3DDPG agent
...
# save testing stats
if isDeterministic:
    path = f'stats/test/m3ddpg_d_{episode}.pkl'
else:
    path = f'stats/test/m3ddpg_{episode}.pkl'
